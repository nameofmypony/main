...
class Program
{
    ...
    static void Main()
    {
        ...
        else
        {
            ...
            for (int i = 0; i < h; i++) for (int j = 0; j < w; j++) image[i, j] = color;
        }
    ...
    {
        part = part.Trim();
        if (part == "-" || w == 0 || h == 0) return;
        else if (part == "Z") regionsQueue.Enqueue(new Region { x = x, y = y, w = w, h = h });
        else
        {
            int color = int.Parse(part);
            for (int i = y; i < y + h; i++) for (int j = x; j < x + w; j++) image[i, j] = color;
        }
    }
}
