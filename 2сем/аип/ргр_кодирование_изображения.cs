...
class Program
{
    ...
    {
        {
            ...
            for (int i = 0; i < h; i++)
            {
                ...
                for (int j = 0; j < w; j++) image[i, j] = int.Parse(line[j]);
            }
            output = "";
            regionsQueue = new Queue<string>();
            output += $"{w} {h}\n";
            if (IsSameColor(0, 0, w, h)) output += image[0, 0].ToString() + "\n";
            else
            {
                output += "Z\n";
                EncodeRegion(0, 0, w, h);
            }
            while (regionsQueue.Count > 0) output += regionsQueue.Dequeue() + "\n";
    ...
    static bool IsSameColor(int x, int y, int w, int h)
    {
        ...
        int color = image[y, x];
        for (int i = y; i < y + h; i++) for (int j = x; j < x + w; j++) if (image[i, j] != color) return false;
        return true;
    }
    ...
    {
        if (w == 0 || h == 0) return "-";
        else if (IsSameColor(x, y, w, h)) return image[y, x].ToString();
        else return "Z";
    }
}
